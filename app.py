        ...
        context.update(
            {
                ...
                "dollar_tree_relevance": "Dollar Tree sells high-velocity consumables where recalls can require UPC matching, vendor review, and store withdrawal decisions.",
        ...
        context.update(
            {
                ...
                "dollar_tree_relevance": "Dollar Tree's value proposition is sensitive to consumer price pressure and trade-down behavior.",
        ...
        context.update(
            {
                ...
                "dollar_tree_relevance": "Household inflation can influence trade-down into Dollar Tree home and cleaning categories.",
        ...
        context.update(
            {
                ...
                "dollar_tree_relevance": "Retail news can surface competitor moves, price pressure, closures, recalls, and market events relevant to Dollar Tree planning.",
...
def enrich_feature_rows_for_retailer(feature_df: pd.DataFrame, retailer_name: str) -> pd.DataFrame:
    ...
    if "Dollar Tree" not in retailer_name:
        enriched["dollar_tree_relevance"] = enriched["dollar_tree_relevance"].str.replace("Dollar Tree", retailer_name or "the retailer", regex=False)
    return enriched
...
def llm_user_prompt(retailer: str, region: str, payload: Dict[str, Any]) -> str:
    ...
    3. Dollar Tree category impact, clearly stating how the signal can become a forecast feature
    ...
    - Use Dollar Tree-specific language when the payload includes dollar_tree_category, enterprise_kpi, planning_owner, impact_hypothesis, and internal_data_needed.
    """
...
def render_explainability_ladder() -> None:
    steps = [
        ...
        ("03", "Map", "Rows are mapped to Dollar Tree category, owner, KPI, and forecast feature."),
...
def render_dollar_tree_impact_matrix(feature_df: pd.DataFrame) -> None:
    if feature_df.empty:
        st.info("No Dollar Tree impact matrix is available until feature rows are generated.")
        return
    preferred_cols = [
...
def render_scenario_simulator(feature_df: pd.DataFrame) -> None:
    ...
    st.markdown(
        ...
        f"<div class='enterprise-band-copy'><strong>Likely Dollar Tree category:</strong> {escape(scenario['category'])}<br>"
...
if view == "Configure":
    ...
    st.markdown(
        ...
        "<div class='config-copy'>The run can support buyer and planner discussion, but it does not claim internal category performance until Dollar Tree POS, inventory, product hierarchy, vendor, promotion, and store/DC data are connected.</div>"
...
if view == "Results":
    ...
    if not run:
        st.markdown(
            ...
            "<div class='empty-body'>Run the governed signal pipeline to populate decision cards, Dollar Tree impact mapping, score explanations, source evidence, and the executive brief.</div>"
            "</div>"
        ...
        if feature_df.empty:
            ...
            with command_tab:
                ...
                st.markdown('<div class="small-header">Dollar Tree Enterprise KPIs</div>', unsafe_allow_html=True)
                ...
                st.markdown('<div class="small-header">Dollar Tree Impact Matrix</div>', unsafe_allow_html=True)
